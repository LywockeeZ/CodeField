    ...
    for(int y = yMin; y <= yMax; y++)
    {
        for(int x = xMin; x <= xMax; x++)
        {
            if(!isMSAAon)
            {
                ...
                {
                    float z_interpolated = get_interpolated_z(x, y, t);
            ...
            else
            {
                ...
                {
                    Eigen::Vector3f point = Eigen::Vector3f(x, y, 1.0f);
                    set_pixel(point, t.getColor() * count / 4.0f + (4 - count) * frame_buf[get_index(x, y)] / 4.0f);
                }
            }
...
float rst::rasterizer::get_interpolated_z(float x, float y, const Triangle& t)
{
    auto v = t.toVector4();
    auto[alpha, beta, gamma] = computeBarycentric2D(x, y, t.v);
    float w_reciprocal = 1.0/(alpha / v[0].w() + beta / v[1].w() + gamma / v[2].w());
...
int rst::rasterizer::get_depth_index(int x, int y)
{
...
void rst::rasterizer::openMSAA() 
{
     isMSAAon = true; 
     depth_buf.resize(2 * width * 2 * height); 
}

void rst::rasterizer::closeMSAA() 
{ 
    isMSAAon = false; 
