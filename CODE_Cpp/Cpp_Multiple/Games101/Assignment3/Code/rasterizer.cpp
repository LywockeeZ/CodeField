...
void rst::rasterizer::draw(std::vector<Triangle *> &TriangleList) {
    ...
    for (const auto& t:TriangleList)
    {
        ...
        //to make the normal not changed by the scale and translate transform
...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    for(int y = yMin; y <= yMax; y++)
    {
        for(int x = xMin; x <= xMax; x++)
        {
            ...
            {
                //we have converted the z to screen space, so we must correct it, but the other attribute is
                //in view space ,so we can use the lerp result directly.
                ...
                if(zp < depth_buf[get_index(x, y)])
                {
                    ...
                    //calculate color by specific fragment shader
