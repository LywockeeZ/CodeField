...
def getInliers(f1, f2, matches, M, RANSACthresh):
    ...
    for i in range(len(matches)):
        ...
        vec2 = np.array([[0, 0, 1]])
        m = matches[i]
        (vec1[0][0], vec1[0][1]) = f1[m.queryIdx].pt
        (vec2[0][0], vec2[0][1]) = f2[m.trainIdx].pt
        vec3 = np.dot(M, vec1.T)
        vec4 = vec3.transpose()

        vec2 = vec2/vec2[0][2]
        vec4 = vec4/vec4[0][2]
        euc = np.linalg.norm(vec4.T - vec2.T)
...
def leastSquaresFit(f1, f2, matches, m, inlier_indices):
    ...
    if m == eTranslate:
        ...
        for i in range(len(inlier_indices)):
            ...
            u += (f2[p2].pt[0]-f1[p1].pt[0])
            v += (f2[p2].pt[1]-f1[p1].pt[1])
