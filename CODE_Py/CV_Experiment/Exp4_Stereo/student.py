...
def compute_photometric_stereo_impl(lights, images):
    ...
    albedo = np.zeros((height, width, _))
    normals = np.zeros((height, width, 3))
    N = np.zeros([3, 1], float)
    images_arr = np.array(images)
    for i in range(height):
        for j in range(width):
            for k in range(_):
                I = images_arr[:, i, j, k].reshape(length, 1)
                G = np.dot(np.linalg.inv(np.dot(lights.T, lights)), np.dot(lights.T, I)).reshape(3, 1)
                kd = np.linalg.norm(G)
                kd = 1 if kd == 0 else kd
                albedo[i, j, k] = kd
                N = (G * (1. / kd)).reshape(3)
            normals[i, j, 0] = N[0]
            normals[i, j, 1] = N[1]
...
def project_impl(K, Rt, points):
    ...
    for i in range(height):
        for j in range(width):
            _x, _y, _z = points[i, j, :]
            u = np.dot(PI, np.array([_x, _y, _z, 1]).reshape(4, 1))
            u = u / u[2, 0]
...
def preprocess_ncc_impl(image, ncc_size):
    ...
    """
    # raise NotImplementedError()
    border = int(ncc_size / 2)
    height, width, channals = image.shape
    normalized = np.zeros((height, width, (channals * ncc_size ** 2)))

    for i in range(height):
        for j in range(width):
            if (i - border) >= 0 and j - border >= 0 and i + border < height and j + border < width:

                temp = image[(i - border):(i + border + 1), (j - border):(j + border + 1), :]
                means = np.mean(np.mean(temp, axis=1), axis=0)
                patch = temp - means
                patch_t = np.sqrt(np.sum(patch ** 2))
                patch_t = 1 if patch_t == 0 else patch_t
                pre = patch / patch_t
                patch = np.vstack(pre)
